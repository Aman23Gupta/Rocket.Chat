 ...
 * @type {number}
 */
var MIN_WIDTH = 200; // var MIN_WIDTH = 790;
...
function JitsiMeetExternalAPI(domain, room_name, width, height, parentNode, configOverwrite, interfaceConfigOverwrite, noSsl, token) {
  ...
  if (configOverwrite) {
    for (key in meteorBabelHelpers.sanitizeForInObject(configOverwrite)) {
  ...
  if (interfaceConfigOverwrite) {
    for (key in meteorBabelHelpers.sanitizeForInObject(interfaceConfigOverwrite)) {
...
JitsiMeetExternalAPI.prototype.executeCommands = function (object) {
  ...
  Object.entries(object).forEach(function (_ref) {
    ...
    return _this.executeCommand(key, value);
  });
};
/**
...
JitsiMeetExternalAPI.prototype.addEventListeners = function (object) {
  ...
  Object.entries(object).forEach(function (_ref3) {
    ...
    return _this2.addEventListener(key, value);
  });
};
/**
...
JitsiMeetExternalAPI.prototype.addEventListener = function (event, listener) {
  ...
  if (!(event in this.eventHandlers)) {
    this.postis.listen(events[event], function (data) {
      if (event in _this3.eventHandlers && typeof _this3.eventHandlers[event] === 'function') {
        _this3.eventHandlers[event].call(null, data);
      }
    });
  }
...
//# sourceMappingURL=/dynamic/client/views/room/contextualBar/Call/Jitsi/lib/616c278f1254909dbeb611eabe281266b6720ece.map
